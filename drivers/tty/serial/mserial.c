...
#define MSERIAL_CONSTAT		0x04
#define MSERIAL_TXDATA		0x00
...
static void mserial_putchar(struct uart_port *port, int ch) {
	iowrite8(ch, port->membase + MSERIAL_TXDATA);
}

static int mserial_transmit(struct uart_port *port)
{
    struct circ_buf *xmit  = &port->state->xmit;


	if (port->x_char) {
		mserial_putchar(port, port->x_char);
		port->x_char = 0;
		port->icount.tx++;
		return 1;
	}
	...
	mserial_putchar(port, xmit->buf[xmit->tail]);
...
static enum hrtimer_restart mserial_poll(struct hrtimer *timer)
{
	...
	do {
		rx_or_tx = mserial_transmit(port);
		n++;
	} while (rx_or_tx);
...
static void
mfocserial_console_write(struct console *co, const char *s, unsigned int count)
{
	struct uart_port *port = &mserial_ports[co->index];

	uart_console_write(port, s, count, mserial_putchar);
}


static int mfocserial_console_setup(struct console *co, char *options)
{
	struct uart_port *port;
	int baud = 115200;
	int bits = 8;
	 ...
	 * console support.
	 */
	if (co->index >= MSERIAL_NR_UARTS)
		co->index = 0;
	port = &mserial_ports[co->index];
	if (!port)
		return -ENODEV;
...
static int mfocserial_console_match(struct console *co, char *name, int idx,
			       char *options)
{
	...
	co->index = 0;
	port->cons = co;
	return mfocserial_console_setup(co, options);
}

static void mfocserial_early_write(struct console *con, const char *s, unsigned n)
{
	struct earlycon_device *dev = con->data;

	uart_console_write(&dev->port, s, n, mserial_putchar);
}

static int __init mfocserial_early_console_setup(struct earlycon_device *device,
					    const char *opt)
{
	if (!device->port.membase)
		return -ENODEV;

	device->con->write = mfocserial_early_write;
	device->con->read = NULL;

	return 0;
}
OF_EARLYCON_DECLARE(mserial, "mfoc,mfocserial", mfocserial_early_console_setup);
...
static struct uart_driver mserial_uart_driver;
static struct console mfocserial_console = {
    .name        = "ttyMFOC",
    .write       = mfocserial_console_write,
    .device      = uart_console_device,
    .setup       = mfocserial_console_setup,
	.match		= mfocserial_console_match,
    .flags       = CON_PRINTBUFFER,
    .index       = -1,
    .data        = &mserial_uart_driver,
};

static int __init mfocserial_console_init(void)
{
   	printk(KERN_INFO "Serial: MSERIAL UART driver\n");
    return 0;
}


console_initcall(mfocserial_console_init);
			  ...
			  struct mserial_data *pdata)
{
    ...
    pr_debug("mfocserial: registered port: %s%i\n", MSERIAL_NAME, id);
